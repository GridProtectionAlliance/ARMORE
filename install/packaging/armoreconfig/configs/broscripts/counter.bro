...
function process_result(ts: time, key: SumStats::Key, result: SumStats::Result)
{
            ...
            local src: addr;
            local dst: addr;
            local indexes: string_array;
            local pro: string;
            local fun: string;
            local tgt_string: string;
            local sub_tgts: string_array;
            local v: vector of count;
            local ind: count;
            local tgt: table[string] of count;
            local sub_tgt: string_array;
            local total = 0;
            ...
            if(levels >= 2)
            {
                if("receivers" in result)
                {
                    ...
                    {
                        indexes = split(i, /;/);
                        src = to_addr(indexes[1]);
                        dst = to_addr(indexes[2]);
                        if ( ! map[src]?$value )
                        {
            ...
            if (levels >= 5)
            {
                if("modbus_tgt" in result)
                {
                    ...
                    for ( i in counttable )
                    {
                        ...
                        sub_tgts = split(tgt_string, / /);
                        v = range(1, |sub_tgts|);
                        ind = 1;
                        tgt = table();
                        for (j in v)
                        {
                            sub_tgt = split(sub_tgts[ind], /:/);
                            tgt[sub_tgt[1]] = to_count(sub_tgt[2]);
                            ++ind;
                        }
