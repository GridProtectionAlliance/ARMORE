...
function checkPolicy(c: connection, t: string) {
	...
	if (f$proto == "*") {
	   ...
	   if (c?$modbus) {
        ...
        if ((to_lower(f$proto) == "modbus" && c?$modbus) || (to_lower(f$proto) == "dnp3" && c?$dnp3)) {
            protoFlagged = T;
            reason += fmt("Protocol=%s,",  f$proto);
        }

		# Function - Modbus
		if (to_lower(f$proto) == "modbus" || f$proto == "*") {
			if (c?$modbus && c$modbus?$func) {
				if (to_lower(f$funcRegex) == "contains") {
					if (strstr(to_lower(c$modbus$func), to_lower(f$func)) != 0 || f$func == "*") {
						funcFlagged = T;
						reason += fmt("Function=%s,", c$modbus$func);
					}
				} else if (to_lower(c$modbus$func) == to_lower(f$func)) {
					funcFlagged = T;
					reason += fmt("Function=%s,", c$modbus$func);
				}
			}
		...
		# Function - DNP3
		if (to_lower(f$proto) == "dnp3" || f$proto == "*") {
			if (c?$dnp3) {
				if (to_lower(f$funcRegex) == "contains") {
					if (c$dnp3?$fc_request && (strstr(to_lower(c$dnp3$fc_request), to_lower(f$func)) != 0)) {
						funcFlagged = T;
						reason += fmt("Function=%s,", c$dnp3$fc_request);
					}
					if (c$dnp3?$fc_reply && (strstr(to_lower(c$dnp3$fc_reply), to_lower(f$func)) != 0)) {
						funcFlagged = T;
						reason += fmt("Function=%s,", c$dnp3$fc_reply);
					}
				} else {
					if (c$dnp3?$fc_request && c$dnp3$fc_request == f$func) {
						funcFlagged = T;
						reason += fmt("Function=%s,", c$dnp3$fc_request);
					}
					if (c$dnp3?$fc_reply && c$dnp3$fc_reply == f$func) {
						funcFlagged = T;
						reason += fmt("Function=%s,", c$dnp3$fc_reply);
					}
				}
...
function checkPolicyDnp3(rec: DNP3::Info) {
	for (f in filtersDnp3) {
		...
		# Function - DNP3
		if (to_lower(f$funcRegex) == "contains") {
			if (rec?$fc_request && (strstr(to_lower(rec$fc_request), to_lower(f$func)) != 0)) {
				funcFlagged = T;
				reason += fmt("Function=%s,", rec$fc_request);
			}
			if (rec?$fc_reply && (strstr(to_lower(rec$fc_reply), to_lower(f$func)) != 0)) {
				funcFlagged = T;
				reason += fmt("Function=%s,", rec$fc_reply);
			}
		} else {
