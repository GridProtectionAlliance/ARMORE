...
# Create database
DB_NAME_ARMORE="/var/webServer/database/armore.db";
cat /dev/null > $DB_NAME_ARMORE;
...
sqlite3 $DB_NAME_ARMORE < $TMP_FILE;
rm -f $TMP_FILE;
...
sqlite3 $DB_NAME_ARMORE "$ADD_ROLES_USER";
sqlite3 $DB_NAME_ARMORE "$ADD_ROLES_ADMIN";
